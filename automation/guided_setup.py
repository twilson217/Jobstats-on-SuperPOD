...
python3 /tmp/update_slurm_conf.py''',
        ...
        # Get BCM headnode for cmsh commands (use slurm_controller as BCM headnode)
        bcm_headnode = self.config['systems']['slurm_controller'][0] if self.config['systems']['slurm_controller'] else 'slogin'
        cluster_name = self.config['cluster_name']
        
        cgroup_cmsh_commands = [
            {
                'host': bcm_headnode,
                'command': f'cmsh -c "wlm;use {cluster_name};cgroups;set constrainramspace yes;commit"',
                'description': f'Enable RAM space constraints for {cluster_name} cluster via BCM'
            },
            {
                'host': bcm_headnode,
                'command': f'cmsh -c "wlm;use {cluster_name};cgroups;set constraincores yes;commit"',
                'description': f'Enable CPU core constraints for {cluster_name} cluster via BCM'
            },
            {
                'host': bcm_headnode,
                'command': f'cmsh -c "wlm;use {cluster_name};set selecttypeparameters CR_CPU_Memory;commit"',
                'description': f'Configure select type parameters for cgroup support on {cluster_name} cluster via BCM'
            }
        ]
    ...
    def section_bcm_configurations(self):
        ...
        for host, roles in hosts_by_role.items():
            ...
            imaging_instructions.append({
                'host': host,
                'node_type': node_type,
                'command': f"cmsh -c 'device;use {host};grabimage -w'"
            })
