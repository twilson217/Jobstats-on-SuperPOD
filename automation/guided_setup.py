...
class GuidedJobstatsSetup:
    ...
    def section_cgroups(self):
        ...
        self._add_to_document("")
        self._add_to_document("The following settings need to be added to slurm.conf:")
        ...
        self._add_to_document("```")
        self._add_to_document("")
        self._add_to_document("**Note:** Add these settings before the AUTOGENERATED SECTION to maintain BCM compatibility.")
        self._add_to_document("")
        ...
        # For actual execution, use the Python script approach
        if not self.dry_run:
            update_commands = [
                {
                    'host': slurm_controller,
...
rm -f /tmp/update_slurm_cgroups.py''',
            ...
            # For dry-run documentation, use simple vim instructions
            update_commands = [
                {
                    'host': slurm_controller,
                    'command': f'''# Backup original slurm.conf
...
# Edit slurm.conf to add cgroup settings
echo "Edit {slurm_conf_path} with vim or your preferred editor"
echo "Add the following lines before the AUTOGENERATED SECTION:"
echo "JobAcctGatherType=jobacct_gather/cgroup"
echo "ProctrackType=proctrack/cgroup"
echo "TaskPlugin=affinity,cgroup"
echo ""
echo "Example vim command: vim {slurm_conf_path}"''',
                    'description': 'Update slurm.conf with cgroup settings (manual edit)'
                }
            ]
...
EOF''',
    ...
    def section_gpu_scripts(self):
        ...
        self._add_to_document("### What we'll do")
        self._add_to_document("")
        self._add_to_document("- Clone jobstats repository")
        self._add_to_document("- Install prolog/epilog scripts on GPU nodes")
        self._add_to_document("- Install NVIDIA GPU exporter")
        self._add_to_document("- Create systemd service for GPU exporter")
        self._add_to_document("- Verify GPU exporter is running and collecting metrics")
        self._add_to_document("")
        ...
        print(f"• Install NVIDIA GPU exporter")
        print(f"• Create systemd service for GPU exporter")
        print(f"• Verify GPU exporter is running and collecting metrics")
...
EOF''',
        ...
        if not self._execute_commands(gpu_commands, "GPU Scripts and Exporter Installation"):
            print(f"\n{Colors.RED}✗ GPU scripts and exporter installation failed{Colors.END}")
            return False
        
        # Step 2: Verify GPU exporter installation
        print(f"\n{Colors.BOLD}{Colors.YELLOW}Step 2: Verifying GPU exporter installation{Colors.END}")
        ...
        print(f"\n{Colors.BOLD}{Colors.GREEN}✓ GPU scripts and exporter installation completed{Colors.END}")
        print(f"{Colors.BOLD}{Colors.GREEN}✓ GPU exporter verification completed{Colors.END}")
